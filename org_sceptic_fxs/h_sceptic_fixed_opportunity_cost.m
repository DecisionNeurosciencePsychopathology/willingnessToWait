function  [fx] = h_sceptic_fixed_opportunity_cost(x_t, theta, u, inF)
...
%Compute elig traces
e = compute_elig_trace(rt,sig_spread,inF,refspread,nbasis);
choice_e = compute_elig_trace(rt_transformed,sig_spread,inF,refspread,nbasis);

%asymptote the elig trace for value 9/13 use local values only!
%e(find(e==max(e)):end) = max(e);

%1) compute prediction error, scaled by eligibility trace
    delta = e.*(reward - x_t(1:nbasis));

fx(1:nbasis) = x_t(1:nbasis) + alpha.*delta;
fx(nbasis+1:nbasis*2) = gamma.*x_t(nbasis+1:nbasis*2) + choice_e;

fx = fx';


function e = compute_elig_trace(rt,sig_spread,inF,refspread,nbasis)
